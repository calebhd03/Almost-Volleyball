...
internal void draw_ball_in_pixels(int x_pos, int y_pos, int radius, u32 color)
{
	int x0 = clamp(0, x_pos - radius, render_state.width);
	int x1 = clamp(0, x_pos + radius, render_state.width);
	int y0 = clamp(0, y_pos - radius, render_state.height);
	int y1 = clamp(0, y_pos + radius, render_state.height);
	...
	for (int y = y0; y < y1; y++)
	{
		...
		for (int x = x0; x < x1; x++)
		{
			if(distance(x_pos, y_pos, x, y) <= radius)
				*pixel++ = color;
		}
	}
	draw_rect(x_pos, y_pos, 10, 10, color_red);
}
